...
def hit():
    ...
    for i in range(0, len(dealer_hand)):
        ...
        if int(sum2) > 21 and 'A of Spades' in dealer_hand:
            deck_dict['A of Spades'] = 1
        if int(sum2) > 21 and 'A of Hearts' in dealer_hand:
            deck_dict['A of Hearts'] = 1
        if int(sum2) > 21 and 'A of Clubs' in dealer_hand:
            deck_dict['A of Clubs'] = 1
        if int(sum2) > 21 and 'A of Diamonds' in dealer_hand:
    ...
    if int(sum1) > 21 and 'A of Spades' in player_hand:
        deck_dict['A of Spades'] = 1
    if int(sum1) > 21 and 'A of Hearts' in player_hand:
        deck_dict['A of Hearts'] = 1
    if int(sum1) > 21 and 'A of Clubs' in player_hand:
        deck_dict['A of Clubs'] = 1
    if int(sum1) > 21 and 'A of Diamonds' in player_hand:
        deck_dict['A of Diamonds'] = 1
    sum1 = 0
    for i in range(0, len(player_hand)):
        sum1 = sum1 + (deck_dict[player_hand[i]])
    if int(sum1) > 21:
        props = {
        "message": "your hand is " + str(player_hand) + "the count in your hand is " + str(sum1),
        "message2": "the dealer's hand is " + str(dealer_hand) + "the count in the dealer's hand is " + str(sum2),
        "message3": "YOU LOSE!",
        "pcards": player_image,
        "dcards": dealer_image
        }
